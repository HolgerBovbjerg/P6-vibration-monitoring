...
Uint16 localCounter1; // Counter variables for monitoring real-time operation.
Uint16 localCounter2;
...
void aic3204_codec_read(Int16* left_input, Int16* right_input)
{
	...
	localCounter1 = 0; // monitoring real-time operation
	
	/* Read Digital audio inputs */
    while(!(I2S2_IR & RcvR) ) 			// while no interrupt flag register and no 
    {
    	localCounter1++; // Wait for receive interrupt
    }	
...
{
	localCounter2 = 0;  // monitoring real-time operation
    while( !(I2S2_IR & XmitR) ) // while no interrupt flag register and no 
    {
   		localCounter2++; // Wait for transmit interrupt
    }	
